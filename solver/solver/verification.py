...
def convergence_order(h1: float, h2: float, e1: float, e2: float) -> float:
    """Observed order p for e proportional to h**p."""
    if min(h1, h2, e1, e2) <= 0:
        raise ValueError("grid spacings and errors must be positive")
