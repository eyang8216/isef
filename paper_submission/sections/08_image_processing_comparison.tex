...
\begin{enumerate}
    ...
    \item Select $N$ representative frames (e.g., $N=10$) uniformly spaced within the stable interval.
    \item Average the extracted profiles across these frames to reduce random noise.
\end{enumerate}
...
\begin{enumerate}
    \item Run the solver at a known geometry (e.g., electrode spacing $L = \SI{10}{mm}$, needle diameter $d = \SI{0.8}{mm}$, surface tension $\gamma = \SI{0.022}{N/m}$, applied voltage $V = \SI{3}{kV}$).
    ...
    \item Embed the silhouette in a realistic image background: add a needle representation, set pixel resolution (e.g., 1024$\times$768), apply calibration scale (e.g., \SI{50}{\micro m/pixel}), render a backlight gradient.
...
If the synthetic validation fails these criteria, the edge-detection method, smoothing parameters, or flank-window selection will be revised and re-validated. Only after the synthetic validation passes will the pipeline be applied to experimental images. The synthetic validation results (images, extracted profiles, error statistics) will be included in the final paper or supplementary materials as evidence that the image-processing method is unbiased and sufficiently accurate.
