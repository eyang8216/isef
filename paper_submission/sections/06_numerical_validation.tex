...
\end{equation}
is verified by imposing the exact analytic Taylor potential on the domain boundary with the rounded cone as the zero equipotential. The measured amplitude ratio is within 3\% of unity (grid-dependent, $\approx 1.01$), and the projected-residual argmin occurs within approximately \SI{2}{\degree} of the ideal \SI{49.29}{\degree}. This small systematic offset is attributed to the cap-flank geometry and the $\approx\pm1^{\circ}$ discretization resolution floor, and is stable under grid refinement. The strong result is the amplitude identity itself.
