...
with col_run:
    run = st.button("▶️ Run Solver", type="primary")
...
if result is None:
    st.info(
        "👈 Configure parameters in the sidebar and click **▶️ Run Solver** to start.",
        icon="ℹ️"
    )
else:
    ...
    if st.session_state.last_params != params:
        st.warning(
            "⚠️ Showing results from the previous run — some parameters have changed. "
            "Click **▶️ Run Solver** to recompute.",
            icon="⚠️"
        )
    ...
    if result.half_angle < params.interface_half_angle_deg - 0.01:
        st.warning(
            f"⚠️ The requested interface angle {params.interface_half_angle_deg:.1f}° exceeds "
