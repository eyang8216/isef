...
# Add custom CSS for highlighting novel contributions
st.markdown("""
<style>
.novelty-highlight {
...
with st.sidebar:
    ...
    **Taylor far-field** boundary condition is used: the exact analytical Taylor potential
    is imposed on the outer boundary. This is the correct verification test (recovers ≈49.2° → 49.29°, icon="ℹ️").
    """)
    ...
    if verif_nr * verif_nz > 30000:
        st.warning(
            f" Very Fine grid has {verif_nr * verif_nz:,} cells. "
...
with col_run:
    run_verif = st.button(" Run Verification", type="primary")
...
if run_verif:
    try:
        with st.spinner("Running immersed verification... This may take 10-60 seconds..."):
            ...
            progress_placeholder.info("Sweeping cone angles and computing flank field exponents...")
...
if verif is None:
    st.info(
        " Configure verification parameters in the sidebar and click ** Run Verification** to start."
    , icon="ℹ️")
else:
    # Check if grid changed
    if verif.grid_r.size != verif_nr or verif.grid_z.size != verif_nz:
        st.warning(
            " Showing results from the previous run — the grid has changed. "
            "Click ** Run Verification** to recompute."
        , icon="⚠️")
    ...
    if verif.identity_ratio is not None:
        st.markdown('<div class="novelty-highlight"><span class="novelty-title">Novel Contribution:</span> Exponent Crossing Observable</div>', unsafe_allow_html=True)
        ...
        if abs(verif.identity_ratio - 1.0) < 0.01:
            ...
            st.info(
                f"Good agreement: Identity ratio within 5% of exact (measured {verif.identity_ratio:.4f}). "
                "Consider using a finer grid for better accuracy.",
                icon="ℹ️"
            )
        else:
            st.warning(
                f"Significant deviation: Identity ratio is {abs(verif.identity_ratio - 1.0) * 100:.1f}% "
    ...
    if angle_error < 0.5:
        ...
        st.info(
            f"Good angle recovery: {verif.recovered_angle_deg:.2f}° is within 2° of "
            "the theoretical Taylor angle (49.29°). Consider using Very Fine grid for better accuracy.",
            icon="ℹ️"
        )
    else:
        st.warning(
            f"Angle recovery deviation: {verif.recovered_angle_deg:.2f}° differs by "
